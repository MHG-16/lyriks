...
const SongSearchCard = ({ song, isPlaying, activeSong, i, data }) => {
  ...
  "
    >
      ...
      ${activeSong?.title === song?.title ? 'flex bg-black bg-opacity-70' : 'hidden'}`}
        ...
        <img alt="song_img" src={song.song_art_image_url} />
      </div>
      ...
      <div className="mt-4 flex flex-col">
        ...
        <p className="text-sm truncate text-gray-300 mt-1">
          <Link to={song.artist_names ? `/artists/${song.primary_artist.id}`
            : '/top-artists'}
          >
            {song?.artist_names}
          </Link>
